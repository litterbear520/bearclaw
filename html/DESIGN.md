...
### Windows + Chinese (CJK) Adaptation
On Windows with Chinese-heavy content, Latin serif faces (Georgia, Garamond) fall back to **SimSun (宋体)** for CJK glyphs, creating a jarring mismatch. The proven workaround:

- **Display headlines:** `"Microsoft YaHei", "PingFang SC", "Segoe UI", system-ui, sans-serif` at **weight 600** with **letter-spacing -0.3px to -0.5px**. The heavier weight + tight tracking preserves the editorial density that serif display would provide, without the CJK fallback problem.
- **Body text:** `"Segoe UI", -apple-system, system-ui, "PingFang SC", "Microsoft YaHei", sans-serif` at weight 400. Segoe UI is the best humanist sans on Windows — closest feel to Inter/StyreneB.
- **Code:** `"Cascadia Code", "JetBrains Mono", "Fira Code", Consolas, monospace`. Cascadia Code is native to Windows and has excellent CJK alignment.

This stack renders consistently across Windows 10/11 (Microsoft YaHei), macOS (PingFang SC), and Linux (system-ui fallback). The key insight: differentiate display from body via **size + weight + tracking**, not via serif-vs-sans, when the content mixes Latin and CJK scripts.
